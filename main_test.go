...
func TestPrompt(t *testing.T) {
	...
	// Convert the output to a string and compare it to the expected value
	if string(out) != expected {
		t.Errorf("%q expected but got %q", expected, out)
	}
}
...
func TestIntro(t *testing.T) {
	...
	// Restore stdout and read the output from the pipe
	w.Close()
	os.Stdout = oldStdout
	out, _ := io.ReadAll(r)
